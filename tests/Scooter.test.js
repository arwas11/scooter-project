...
describe('scooter methods', () => {
  ...
  test('rent fails due to low battery', () => {
    const scooter = new Scooter('NYC');
    scooter.charge = 15;
    const user = new User('test', 'test123', 20);
    expect(() => scooter.rent(user))
      .toThrow('scooter needs to charge');
    expect(scooter.user).toBeNull();
    expect(scooter.station).toBe('NYC');
  });

  test('rent fails due to broken scooter', () => {
    const scooter = new Scooter('NYC');
    scooter.isBroken = true;
    const user = new User('test', 'test123', 10);
    expect(() => scooter.rent(user))
  ...
  test('dock succeeds', () => {
    ...
    const user = new User('test', 'test123', 40);
    scooter.rent(user);
    scooter.dock('West Loop');
    expect(scooter.user).toBeNull();
    expect(scooter.station).toBe('West Loop');
  });
